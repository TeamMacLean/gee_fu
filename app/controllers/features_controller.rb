...
class FeaturesController < ApplicationController
      ...
      parents.each do |label, parentFeature_gff_id|
        ...
        if (parentFeats)
          parentFeats.each do |pf|
            parent = nil
            parent = Parent.where(parent_feature: pf.id).first #first?
  ...
  def summary
    ...
    flash[:error] << 'End value must be greater than (or equal to) start value' if _end.to_i < start.to_i
...
#   end
  ...
  def genome_typeahead
    ...
    @reference = Genome.find(genome_id).references.where("name LIKE :prefix", prefix: "#{query}%").limit(10).uniq.pluck(:name)
  ...
  def dalliance_entry_points
    ...
    @highest = features.max_by(&:end).end

    # find feature with highest start point
    @lowest = features.min_by(&:start).start
